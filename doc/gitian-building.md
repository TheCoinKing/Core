...
*Setup instructions for a gitian build of coinking using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the coinking
...
- [Building coinking](#building-coinking)
...
Clone the git repositories for coinking and gitian,
...
In [doc/release-process.md](release-process.md) in the coinking repository under 'Fetch and build inputs'.
...
To build coinking (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the coinking repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coinking/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/coinking-project/coinking
...
For example:
```bash
URL=https://github.com/user/coinking.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit coinking=${COMMIT} --url coinking=${URL} ../coinking/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url coinking=${URL} ../coinking/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/coinking-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coinking-build.assert
```
...
[coinking/gitian.sigs](https://github.com/coinking-project/gitian.sigs/) repository, or
