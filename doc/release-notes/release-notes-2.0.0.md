coinking Core version 2.0.0 is now available from:
...
earlier versions of coinking, then run the installer (on Windows) or just copy
over /Applications/coinking-Qt (on Mac) or coinkingd/coinking-qt (on Linux).
...
On Windows, do not forget to uninstall all earlier versions of the coinking
...
Rebranding to coinking Core
---------------------------

To reduce confusion between coinking-the-network and coinking-the-software we
have renamed the reference client to coinking Core.
...
Using the standard "./autogen.sh; ./configure; make" to build coinking-Qt and
coinkingd makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 2.0 release is moving away from the coinkingd executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running coinking daemon to do THIS") was split into a separate
executable, 'coinking-cli'. The RPC client code will eventually be removed from
coinkingd, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let coinkingd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with coinkingd
...
- Rebrand to `coinking Core`
- Move initialization/shutdown to a thread. This prevents "Not responding"
  messages during startup. Also show a window during shutdown.
- Don't regenerate autostart link on every client startup
- Show and store message of normal coinking:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like 
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix coinking-qt startup crash when clicking dock icon
- Linux: Fix Gnome coinking: URI handler
...
- Add separate coinking-cli client
