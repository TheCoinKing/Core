...
---------------------
	...
	mkdir -p ~/.coinking
	echo "rpcuser=username" >>~/.coinking/coinking.conf
	echo "rpcpassword=`head -c 32 /dev/urandom | base64`" >>~/.coinking/coinking.conf
	git clone https://github.com/coinking/coinking
	cd coinking
	git checkout v0.6.0tkc
	qmake -qt=qt5 && make
...
Note: Currently, there is no coinking ebuild available in overlay 
