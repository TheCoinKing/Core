...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("coinking version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  coinkingd [options]                     " + "\n" +
                  "  coinkingd [options] <command> [params]  " + _("Send command to -server or coinkingd") + "\n" +
                  "  coinkingd [options] help                " + _("List commands") + "\n" +
                  "  coinkingd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "coinking:") && !boost::algorithm::istarts_with(argv[i], "tkcoin:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: coinking.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: coinkingd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // tkcoin: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (mapArgs.count("-checkpointkey")) // tkcoin: checkpoint master priv key
    {
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. coinking is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("coinking version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "coinking server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of coinking") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart coinking to complete") << "\n";
