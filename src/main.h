...
static const uint256 hashGenesisBlockOfficial("0x00000000c242d9ab8abce83c4d370bc8e088f7da6f8988075540c556acd2b147");
...
public:
    ...
    {
        // tkcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // tkcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // tkcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // tkcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // tkcoin: whether transaction is a coinstake
    bool fCoinStake;

    // tkcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // tkcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // tkcoin
            undo.nTime = nTime;            // tkcoin
        }
        return true;
    }
...
public:
    ...
    // tkcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // tkcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // tkcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // tkcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // tkcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (tkcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // tkcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // tkcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // tkcoin: block index flags
    enum
    {
