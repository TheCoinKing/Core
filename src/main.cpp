...
// Copyright (c) 2011-2015 The coinking developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_tkc;
...
const string strMessageMagic = "coinking Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // tkcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // tkcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // tkcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // tkcoin: subsidy is cut in half every 16x multiply of difficulty
...
// tkcoin: find last block index up to pindex
...
#endif
{
    ...
    // tkcoin: target change every block
    // tkcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // tkcoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // tkcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // tkcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // tkcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // tkcoin
                    coins.nTime = undo.nTime;           // tkcoin
                } else {
                    if (coins.IsPruned())
    ...
    // tkcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // tkcoin: coinbase reward check relocated to CheckBlock()
    ...
    // tkcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // tkcoin: fees are not collected by miners as in bitcoin
    // tkcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// tkcoin: total coin age spent in transaction, in the unit of coin-days.
...
// tkcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // tkcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // tkcoin: record proof-of-stake hash value
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // tkcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // tkcoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // tkcoin: check that the block satisfies synchronized checkpoint
    ...
    // tkcoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // tkcoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // tkcoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // tkcoin: getblocks may not obtain the ancestor block rejected
    ...
    // tkcoin: if responsible for sync-checkpoint send it
...
// tkcoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // tkcoin: calculate stake modifier checksum
    ...
    // tkcoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "coinking", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // tkcoin: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x185eb24d298b140c0441dbfd8abf3a715ceafabd70cad1d50822d42d995700bf"));
        block.print();
        assert(hash == hashGenesisBlock);
        // tkcoin: check genesis block
        {
            CValidationState state;
        ...
        // tkcoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // tkcoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // tkcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // tkcoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // tkcoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // tkcoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // tkcoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // tkcoin: record my external IP reported by peer
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // tkcoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // tkcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // tkcoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("coinkingMiner : proof-of-work not meeting target");

    //// debug print
    printf("coinkingMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("coinkingMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("coinkingMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "tkcoin-stake-minter" : "tkcoin-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            // tkcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running coinkingMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("coinkingMiner terminated\n");
        throw;
    }
}
...
{
    // tkcoin: mint proof-of-stake blocks in the background
