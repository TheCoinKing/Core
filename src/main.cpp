...
#if defined(NDEBUG)
# error "coinking cannot be compiled without assertions."
#endif
...
int64_t CTransaction::nMinTxFee = 1;  // Override with -mintxfee
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying and mining) */
int64_t CTransaction::nMinRelayTxFee = 1;
...
void ThreadScriptCheck() {
    RenameThread("coinking-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // coinking did not enable this BIP 34
        // TBD
        /*
        if (block.nVersion >= 3)
        {
